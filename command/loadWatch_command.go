...
func NewLoadWatchCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "TestWatch",
		Short: "Performs load test on watch operation",
		Run:   runWatcherFunc,
	}
	cmd.Flags().IntVar(&rounds, "rounds", 1, "number of rounds to run")
...
func performWatchOnPrefixes(ctx context.Context, cmd *cobra.Command, round int) {
	...
	prefixes := UniqueStrings(5, noOfPrefixes)
	keys := RandomStrings(10, keyPerPrefix)
